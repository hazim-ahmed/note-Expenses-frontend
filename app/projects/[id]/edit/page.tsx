...
export default function EditProjectPage() {
  ...
  if (isLoading) {
    return (
      <DashboardLayout>
        <div className="p-8 text-center text-slate-400 font-bold">جاري تحميل بيانات المشروع...</div>
      </DashboardLayout>
    );
  }
      ...
      <div className="max-w-3xl mx-auto space-y-6 pb-12">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">تحديث بيانات كود المشروع، ومركز التكلفة والموقع</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm dark:shadow-xl space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">رقم/كود المشروع *</label>
              <input
                type="text"
                required
                value={projectCode}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">الموقع</label>
              <input
                type="text"
                value={location}
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">مدير المشروع</label>
              <select
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">حالة المشروع</label>
              <select
                value={status}
            ...
            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">الميزانية التقديرية (ر.س)</label>
              <input
                type="number"
                step="0.01"
                value={estimatedBudget}
                onChange={(e) => setEstimatedBudget(e.target.value)}
                className="w-full p-2.5 border border-slate-300 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-950 text-sm font-extrabold text-cyan-600 dark:text-cyan-400 font-mono-num focus:ring-2 focus:ring-cyan-500 outline-none"
              />
            </div>
          </div>
